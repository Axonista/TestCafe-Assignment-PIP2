...
class Test1_userinvitationPage {
  constructor() {
    ...
    this.useremailinvitestatus = XPathSelector("//*[@class='Badge mt-sm']");
  ...
  async invitenewUser(testEmail) {
    ...
    const useremail = (await this.useremailinvite.nth(0).innerText).trim();
    const status = (await this.useremailinvitestatus.nth(0).innerText).trim();

    await t
      .expect(useremail).eql(testEmail, `First user email does not match: expected ${testEmail}, found ${useremail}`)
      .expect(status).eql('INVITED', `First user status is not 'INVITED': found ${status}`);
  } 
